...
class VNLinear(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.map_to_feat = nn.Linear(in_channels,
                                     out_channels,
                                     bias=False)
    def forward(self, x):
        '''
...
class VNReLU(nn.Module):
    ...
    def forward(self, x):
        ...
        # ^ because it's divided by the norm twice
        x_out = x + (mask*x + (1-mask)*(x-(dotprod/(d_norm_sq+EPS))*d))
        return x_out
    
class VNLinearReLU(nn.Module):
    def __init__(self, in_channels, out_channels,
                 share_nonlinearity=False):
        super(VNLinearReLU, self).__init__()
        self.linear = VNLinear(in_channels, out_channels)
        self.relu = VNReLU(out_channels, 
...
class VNLeakyReLU(nn.Module):
    def __init__(self, in_channels, share_nonlinearity=False,
        ...
        # map_to_dir maps input features
        # to either a single (if nonlinearity shared)
        # or several vectors
        if share_nonlinearity == True:
            self.map_to_dir = nn.Linear(in_channels, 1, bias=False)
        else:
...
class VNLinearLeakyReLU(nn.Module):
    def __init__(self, in_channels, out_channels, 
                 dim=5, share_nonlinearity=False, 
                 negative_slope=0.2):
        super(VNLinearLeakyReLU, self).__init__()
        self.dim = dim
        self.negative_slope = negative_slope
        
        self.map_to_feat = nn.Linear(in_channels, out_channels, 
                                     bias=False)
        ...
        if share_nonlinearity == True:
            ...
            self.map_to_dir = nn.Linear(in_channels, out_channels, 
                                        bias=False)
...
class VNStdFeature(nn.Module):
    def __init__(self, in_channels, dim=4, normalize_frame=False,
        ...
        self.vn1 = VNLinearLeakyReLU(in_channels, in_channels//2,
                                     dim=dim, share_nonlinearity=share_nonlinearity,
                                     negative_slope=negative_slope)
        self.vn2 = VNLinearLeakyReLU(in_channels//2, in_channels//4,
                                     dim=dim, share_nonlinearity=share_nonlinearity,
                                     negative_slope=negative_slope)
        if normalize_frame:
